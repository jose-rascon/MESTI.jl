...
## Compile MUMPS
Suppose you downloaded the 5.9.0 version of MUMPS to your ~/Downloads/ folder. Then, go to the folder where you want to compile MUMPS, and enter
```shell
tar zxvf ~/Downloads/MUMPS_5.9.0.tar.gz
cd MUMPS_5.9.0
```
in terminal.
...
Read the file <code>INSTALL</code>, copy the closest <code>Makefile.inc</code> file in the <code>Make.inc</code> folder, and modify it to fit your environment and machine. Most importantly, in <code>Makefile.inc</code> you need to specify:
 ...
 - <code>RPATH_OPT</code>: the path to shared libraries that will be built up, such as <code>.../MUMPS_5.9.0/lib/</code>
where <code>...</code> is the path to MUMPS_5.9.0 folder. 
...
```shell
mkdir ~/.julia/config
echo 'ENV["MUMPS_PREFIX"] = ".../MUMPS_5.9.0/lib"' >> ~/.julia/config/startup.jl
```
